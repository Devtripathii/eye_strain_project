...
def _render(phase: str, snap: dict,
    ...
    # ── Feature 3: phase transition banner ──────────────────────────────────
    # Detect a phase change and show the animated banner in ph_focus
    prev_phase = st.session_state.get("_last_rendered_phase", "IDLE")
...
#     padding: 6px 0 20px 0; border-bottom: 1px solid var(--border); margin-bottom: 24px;
# }
# .eyeguard-logo {
#     font-family: 'Space Mono', monospace; font-size: 26px; font-weight: 700;
#     color: var(--cyan); letter-spacing: -1px; text-shadow: 0 0 20px var(--cyan-glow);
# }
# .eyeguard-sub { font-size: 11px; color: var(--text-mute); text-transform: uppercase; letter-spacing: 2px; }
# .version-badge {
#     display: inline-block; background: var(--surface2); border: 1px solid var(--border);
#     color: var(--text-mute); font-family: 'Space Mono', monospace; font-size: 10px;
...
# .metric-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 10px; margin: 10px 0; }
# .metric-card {
#     background: var(--surface); border: 1px solid var(--border); border-radius: 10px;
#     padding: 14px; position: relative; overflow: hidden; transition: border-color 0.2s;
# }
# .metric-card::before {
...
# .metric-label { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: var(--text-mute); margin-bottom: 8px; }
# .metric-value { font-family: 'Space Mono', monospace; font-size: 26px; font-weight: 700; color: var(--text); line-height: 1; }
...
# .comfort-score-big { font-family: 'Space Mono', monospace; font-size: 56px; font-weight: 700; line-height: 1; }
...
# .comfort-label { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: var(--text-mute); margin-top: 6px; }
# .progress-wrap { margin: 16px 0; }
# .progress-label { display: flex; justify-content: space-between; font-size: 12px; color: var(--text-dim); margin-bottom: 8px; }
# .progress-track { background: var(--surface2); border-radius: 999px; height: 6px; overflow: hidden; border: 1px solid var(--border); }
# .progress-fill { height: 100%; border-radius: 999px; background: linear-gradient(90deg, var(--cyan-dim), var(--cyan)); box-shadow: 0 0 12px var(--cyan-glow); transition: width 0.4s ease; }
# .progress-fill.calib { background: linear-gradient(90deg, #1e3a5f, var(--cyan-dim)); }
# .status-pill { display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; border-radius: 999px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; }
...
# .cnn-badge { display: inline-flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 8px 14px; font-family: 'Space Mono', monospace; font-size: 13px; margin: 8px 0; width: 100%; }
...
# .ai-badge { display: flex; align-items: center; gap: 8px; border-radius: 8px; padding: 10px 14px; margin: 6px 0; border-left: 3px solid; font-size: 13px; }
# .ai-badge.anomaly { background: rgba(123,94,167,.12); border-color: var(--purple); color: #b39ddb; }
# .ai-badge.normal  { background: rgba(16,185,129,.08); border-color: var(--green);  color: var(--green); }
# .ai-badge.training{ background: rgba(0,212,255,.06);  border-color: var(--cyan-dim); color: var(--cyan-dim); }
# .forecast-box { background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 12px 14px; margin: 8px 0; }
# .forecast-val { font-family: 'Space Mono', monospace; font-size: 22px; font-weight: 700; margin: 4px 0; }
# .forecast-val.rising  { color: var(--red); }
# .forecast-val.falling { color: var(--green); }
# .forecast-val.stable  { color: var(--cyan); }
# .break-alert { border-radius: 10px; padding: 14px 16px; margin: 12px 0; border-left: 3px solid; }
...
# .focus-card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 20px; margin-bottom: 12px; }
# .focus-card-title { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: var(--cyan); margin-bottom: 10px; font-weight: 600; }
...
# .ear-row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin: 12px 0; }
# .ear-cell { background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 10px 14px; text-align: center; }
# .ear-cell-label { font-size: 9px; text-transform: uppercase; letter-spacing: 1.5px; color: var(--text-mute); margin-bottom: 4px; }
# .ear-cell-value { font-family: 'Space Mono', monospace; font-size: 20px; color: var(--cyan); }
# .fps-chip { display: inline-flex; align-items: center; gap: 4px; background: var(--surface); border: 1px solid var(--border); border-radius: 6px; padding: 3px 10px; font-family: 'Space Mono', monospace; font-size: 11px; color: var(--text-mute); }
# .rec-card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 16px; margin: 12px 0; }
# .rec-title  { font-size: 14px; font-weight: 600; color: var(--text); margin-bottom: 10px; }
# .rec-bullet { display: flex; gap: 8px; font-size: 13px; color: var(--text-dim); margin-bottom: 6px; line-height: 1.5; }
...
# .report-badge { background: rgba(16,185,129,0.15); border: 1px solid var(--green); color: var(--green); border-radius: 6px; padding: 4px 12px; font-size: 12px; font-weight: 600; }
# .history-row { display: grid; grid-template-columns: 160px 1fr 1fr 1fr 1fr auto; gap: 12px; align-items: center; padding: 12px 16px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; margin-bottom: 8px; font-size: 13px; }
...
# div[data-testid="stMetric"] label { color: var(--text-mute) !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 1px !important; }
# div[data-testid="stMetric"] [data-testid="stMetricValue"] { font-family: 'Space Mono', monospace !important; color: var(--text) !important; font-size: 24px !important; }
# button[data-testid="baseButton-primary"] { background: var(--cyan) !important; color: var(--navy) !important; border: none !important; font-weight: 700 !important; font-family: 'DM Sans', sans-serif !important; font-size: 14px !important; border-radius: 8px !important; letter-spacing: 0.5px !important; transition: all 0.2s !important; box-shadow: 0 0 20px var(--cyan-glow) !important; }
# button[data-testid="baseButton-primary"]:hover { box-shadow: 0 0 30px rgba(0,212,255,0.4) !important; transform: translateY(-1px) !important; }
# button[data-testid="baseButton-secondary"] { background: var(--surface) !important; color: var(--text-dim) !important; border: 1px solid var(--border) !important; border-radius: 8px !important; font-family: 'DM Sans', sans-serif !important; }
# button[data-testid="baseButton-secondary"]:hover { border-color: var(--cyan-dim) !important; color: var(--cyan) !important; }
# div[data-testid="stProgress"] > div { background: var(--surface2) !important; border-radius: 999px !important; height: 6px !important; }
# div[data-testid="stProgress"] > div > div { background: linear-gradient(90deg, var(--cyan-dim), var(--cyan)) !important; border-radius: 999px !important; box-shadow: 0 0 10px var(--cyan-glow) !important; }
# div[data-testid="stTextInput"] input, div[data-testid="stNumberInput"] input { background: var(--surface) !important; border: 1px solid var(--border) !important; color: var(--text) !important; border-radius: 8px !important; font-family: 'DM Sans', sans-serif !important; }
...
#         # ── PATCH 2: new AI module instances ─────────────────────────────
#         self.yawn_tracker  = YawnTracker()
#         self.anomaly_det   = PersonalAnomalyDetector()
#         self.forecaster    = FatigueForecaster()
...
#                 self.ear_smoother.reset()
#                 reset_fusion_state()
#                 # ── PATCH 3: reset new AI modules on new session ──────────
#                 self.yawn_tracker.reset()
#                 self.anomaly_det.reset()
#                 self.forecaster.reset()
#                 self._blink_total_count      = 0
...
#         # ── PATCH 4: compute new signals every frame ──────────────────────
#         try:
#             mar      = mouth_aspect_ratio(lms, w, h)
#             fhp_deg  = forward_head_angle(lms, w, h)
#             ear_asym = ear_asymmetry(lms, w, h)
...
#             # ── PATCH 5: IBR, anomaly detection, forecasting ──────────────
#             try:
#                 if blink_occurred:
#                     self._blink_total_count += 1
#                     last_dur = getattr(self.blink_dur, 'last_blink_duration_sec', None)
...
#                     "break_mode": sched_state.mode,
#                     "break_remaining_sec": sched_state.remaining_sec,
#                     # ── PATCH 6: new signal columns in CSV ────────────────
#                     "mar":           round(mar, 4),
#                     "yawn_count":    self.yawn_tracker.yawn_count,
#                     "fhp_angle":     round(fhp_deg, 2),
#                     "fhp_level":     fhp_level(fhp_deg),
...
#                 microsleeps_window=ms_count,
#                 cnn_ewma=self.cnn_ewma, cnn_trend=trend_cnn,
#                 # ── PATCH 7: new signals in live state ───────────────────
#                 mar=mar,
#                 yawn_count=self.yawn_tracker.yawn_count,
#                 fhp_angle=round(fhp_deg, 2),
#                 fhp_level=fhp_level(fhp_deg),
...
# # ─────────────────────────────────────────────────────────────────────────────
# # RENDER  (all UI placeholders filled here)
# # ─────────────────────────────────────────────────────────────────────────────
# def _render(phase: str, snap: dict,
#             ph_focus, ph_progress, ph_ear, ph_comfort,
#             ph_status, ph_break, ph_recs, ph_ai):   # ← ph_ai added
#     t = float(snap.get("t", 0.0))
...
#         section("Focus Task")
#         if phase == "IDLE":
#             st.markdown("""<div class="focus-card">
#                 <div class="focus-card-title">Ready</div>
#                 <div class="focus-card-body">Start the camera, then click
#                 <strong>Start Assessment</strong>.</div></div>""", unsafe_allow_html=True)
#         elif phase == "CALIB":
#             calib_pct = int(min(100, t / CALIB_SECONDS * 100))
...
#                 <div class="focus-card-prompt">Hold still for 10 seconds.</div>
#                 </div>""", unsafe_allow_html=True)
#             ear_live = snap.get("ear")
#             if ear_live is not None:
#                 st.markdown(
#                     f'<div style="font-size:12px;color:var(--text-mute);margin-top:4px">'
#                     f'Live EAR: <span style="color:var(--cyan);font-family:Space Mono,monospace">'
#                     f'{float(ear_live):.4f}</span></div>', unsafe_allow_html=True)
#         elif phase == "RUN":
#             st.markdown(f"""<div class="focus-card">
...
#                 <div class="focus-card-body">See your report below.</div>
#                 </div>""", unsafe_allow_html=True)
...
#             if ev is not None:
#                 st.markdown(f"""<div class="ear-row">
#                     <div class="ear-cell"><div class="ear-cell-label">EAR Smooth</div>
#                         <div class="ear-cell-value">{float(ev):.3f}</div></div>
#                     <div class="ear-cell"><div class="ear-cell-label">EAR Raw</div>
#                         <div class="ear-cell-value">{f"{float(er):.3f}" if er else "—"}</div></div>
#                     <div class="ear-cell"><div class="ear-cell-label">Threshold</div>
#                         <div class="ear-cell-value">{f"{float(et):.3f}" if et else "—"}</div></div>
#                 </div>""", unsafe_allow_html=True)

#     # Comfort score
#     with ph_comfort.container():
#         section("Live Status")
#         c = snap.get("comfort"); css = snap.get("comfort_css", ""); lbl = snap.get("comfort_band", "")
#         if c is None:
#             st.markdown("""<div class="comfort-ring-wrap">
#                 <div class="comfort-score-big" style="color:var(--text-mute)">—</div>
#                 <div class="comfort-label">Awaiting data</div></div>""", unsafe_allow_html=True)
#         else:
#             st.markdown(f"""<div class="comfort-ring-wrap">
#                 <div class="comfort-score-big {css}">{int(c)}</div>
#                 <div class="comfort-label">/ 100 · {lbl}</div></div>""", unsafe_allow_html=True)

#     # Status / FPS / metrics
#     with ph_status.container():
#         fps_val   = float(snap.get("fps", 0.0))
#         fps_color = ("var(--green)" if fps_val >= 18 else "var(--amber)" if fps_val >= 12 else "var(--red)")
#         st.markdown(
#             f'<div style="margin:8px 0 4px 0;font-size:13px;color:var(--text-dim)">'
#             f'{status_pill(phase)} &nbsp; '
#             f'<span class="fps-chip" style="color:{fps_color}">⚡ {fps_val:.0f} fps</span></div>',
#             unsafe_allow_html=True)
...
#                     f'Baseline: <code>{float(be):.4f}</code> · Threshold: <code>{float(et):.4f}</code></div>',
#                     unsafe_allow_html=True)
#             cnn_val = snap.get("cnn_ewma")
#             if cnn_val is None:
#                 st.markdown('<div class="cnn-badge muted">⚪ CNN — warming up</div>', unsafe_allow_html=True)
#             else:
#                 pct = int(cnn_val * 100)
#                 cls, icon = (("danger","🔴") if pct>=65 else ("warn","🟡") if pct>=40 else ("good","🟢"))
#                 st.markdown(f'<div class="cnn-badge {cls}">{icon} CNN · {pct}% sleepiness</div>',
#                             unsafe_allow_html=True)
#             blink = snap.get("blink_rate"); perclos = snap.get("perclos")
#             if blink is not None:
#                 b_css = "danger" if blink<6 else "warn" if blink<12 else "good"
#                 p_css = "danger" if (perclos or 0)>0.20 else "warn" if (perclos or 0)>0.12 else "good"
#                 st.markdown(f"""<div class="metric-grid">
#                     {metric_card("Blink Rate",  f"{blink:.0f}", "/min", b_css)}
#                     {metric_card("Eye Closure", f"{(perclos or 0)*100:.0f}", "%", p_css)}
#                 </div>""", unsafe_allow_html=True)

#     # Break alert
#     with ph_break.container():
#         if phase == "RUN":
#             bm  = snap.get("break_mode", "WORK"); br = float(snap.get("break_remaining_sec", 0.0))
#             st.markdown(f"""<div class="break-alert {_break_css_class(bm)}">
#                 <div class="break-title">{_break_label(bm)}</div>
#                 <div class="break-body">{_break_body(bm, br)}</div>
#             </div>""", unsafe_allow_html=True)
...
#             bullets_html = "".join(f'<div class="rec-bullet">{b}</div>' for b in bullets)
#             st.markdown(f"""<div class="rec-card">
#                 <div class="rec-title">{title}</div>{bullets_html}</div>""",
#                 unsafe_allow_html=True)
#         else:
#             st.markdown('<div style="color:var(--text-mute);font-size:13px">'
#                         'Recommendations appear during assessment.</div>', unsafe_allow_html=True)

#     # ── PATCH 8: AI Intelligence panel ───────────────────────────────────────
#     with ph_ai.container():
#         if phase == "RUN":
#             section("AI Intelligence")
...
#             if not anomaly_trained:
#                 st.markdown(
#                     f'<div class="ai-badge training">⏳ Training your personal baseline… '
#                     f'{int(anomaly_prog*100)}%</div>',
#                     unsafe_allow_html=True)
#             elif anomaly:
#                 st.markdown(
#                     '<div class="ai-badge anomaly">⚠ ANOMALY — Your signals deviate '
#                     'from your personal baseline</div>',
#                     unsafe_allow_html=True)
#             else:
#                 st.markdown(
#                     '<div class="ai-badge normal">✅ Within your normal range</div>',
...
#                                 letter-spacing:1px;">10-min forecast</div>
#                     <div class="forecast-val {fc_trend}">
#                         {trend_icon} {fc_score:.0f}/100 comfort
#                     </div>
...
#             # Adaptive weights display + feedback buttons
#             corrections = snap.get("adaptive_corrections", 0)
#             if corrections > 0:
#                 st.markdown(
#                     f'<div style="font-size:11px;color:var(--cyan);margin:6px 0;">'
#                     f'✦ Model personalised with {corrections} correction(s)</div>',
#                     unsafe_allow_html=True)
...
# # ─────────────────────────────────────────────────────────────────────────────
# def page_profile():
#     st.markdown('<div class="eyeguard-logo" style="font-size:20px;margin-bottom:4px">👤 Profile</div>',
#                 unsafe_allow_html=True)
...
#         st.markdown(f"""<div class="metric-card" style="margin-top:8px">
#             <div class="metric-label">CNN Model Status</div>
#             <div style="font-family:'Space Mono',monospace;font-size:14px;color:var(--cyan)">{cnn_status}</div>
#             <div class="metric-unit">eye_model_best.pth</div></div>""", unsafe_allow_html=True)
#     if st.button("💾  Save Profile", type="primary"):
#         st.session_state.profile = {"name": name.strip() or "User",
#                                      "enable_cnn": enable_cnn, "enable_scheduler": enable_scheduler}
#         profile_path = PROFILE_DIR / f"{name.strip() or 'User'}.json"
#         PROFILE_DIR.mkdir(parents=True, exist_ok=True)
#         with open(profile_path, "w") as f:
#             json.dump(st.session_state.profile, f, indent=2)
...
# # ─────────────────────────────────────────────────────────────────────────────
# def page_history():
#     st.markdown('<div class="eyeguard-logo" style="font-size:20px;margin-bottom:4px">🗂️ History</div>',
#                 unsafe_allow_html=True)
...
# # ─────────────────────────────────────────────────────────────────────────────
# def page_settings():
#     st.markdown('<div class="eyeguard-logo" style="font-size:20px;margin-bottom:4px">⚙️ Settings</div>',
#                 unsafe_allow_html=True)
...
# # ─────────────────────────────────────────────────────────────────────────────
# # REPORT — compute and save  (unchanged except extra CSV columns already added)
# # ─────────────────────────────────────────────────────────────────────────────
# def _compute_and_save_report(proc, cfg: RuntimeCfg):
#     rows = proc.pop_rows()
#     if rows:
...
#     # Extra AI stats from new columns
#     yawn_total = int(df["yawn_count"].max()) if "yawn_count" in df.columns else 0
#     fhp_mean   = round(float(df["fhp_angle"].mean()), 1) if "fhp_angle" in df.columns else None
#     ibr_mean   = round(float(df["ibr"].mean()), 3) if "ibr" in df.columns else None
...
#         <div class="comfort-score-big {fc_css}" style="font-size:40px">{fc}</div>
#         <div>
#             <div style="font-size:11px;color:var(--text-mute);text-transform:uppercase;letter-spacing:1px">Comfort Score</div>
#             <div style="margin-top:6px">
#                 <span class="pill {pill_cls}">{risk} RISK</span>
#                 <span class="report-badge" style="margin-left:8px">Complete ✅</span>
#             </div>
#         </div>
#         <div style="margin-left:auto;font-size:11px;color:var(--text-mute)">{d.get("timestamp","").replace("_"," ")}</div>
#     </div>""", unsafe_allow_html=True)
...
#     st.markdown(f"""<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:16px 0">
...
#     </div>
#     <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:0 0 16px 0">
#         {metric_card("Yawns",        str(yawns),  "this session", "warn" if yawns>=2 else "good")}
#         {metric_card("Head Posture", fhp_str,     "avg FHP",      fhp_css)}
#         {metric_card("Incomplete ↷", ibr_str,     "blink ratio",  ibr_css)}
#     </div>""", unsafe_allow_html=True)

#     ms = d["microsleeps"]; be = d.get("baseline_ear"); et = d.get("ear_threshold")
#     stats = f'Microsleeps: <strong>{ms}</strong>'
#     if be: stats += f' · Baseline EAR: <strong>{be:.3f}</strong> · Threshold: <strong>{et:.3f}</strong>'
#     st.markdown(f'<div style="font-size:13px;color:var(--text-dim);margin:8px 0">{stats}</div>',
#                 unsafe_allow_html=True)

#     bullets_html = "".join(f'<div class="rec-bullet">{b}</div>' for b in d["advice_bullets"])
#     st.markdown(f"""<div class="rec-card" style="margin-top:16px">
#         <div class="rec-title">{d["advice_title"]}</div>{bullets_html}</div>""",
#         unsafe_allow_html=True)

#     st.download_button("⬇  Download Full CSV", data=d["csv_bytes"],
#                        file_name=d["csv_filename"], mime="text/csv", key="report_csv_btn")
...
#                         and webrtc_ctx.state.playing and processor is not None)
#     if not camera_ready:
#         st.markdown('<div style="font-size:13px;color:var(--amber);margin-top:4px">'
#                     '⚠️ Camera not running — click Start above and allow permission.</div>',
#                     unsafe_allow_html=True)
...
#     # ── PATCH 9: Feedback buttons ─────────────────────────────────────────
#     if st.session_state.ui_phase == "RUN" and processor is not None:
#         st.markdown("<br>", unsafe_allow_html=True)
#         st.markdown('<div style="font-size:12px;color:var(--text-mute);margin-bottom:6px;">'
#                     '✦ Correct the AI model — was its assessment accurate right now?</div>',
#                     unsafe_allow_html=True)
#         fb1, fb2, _ = st.columns([2, 2, 2])
#         with fb1:
#             if st.button("✅ I was NOT tired (over-estimated)", key="fb_not_tired"):
...
#         ph_status  = st.empty()
#         ph_break   = st.empty()
#         ph_ai      = st.empty()   # ← new AI panel placeholder
